...
-- for better exploitation of SSE instructions and memory locality when calculating
-- dot products for cross-covariance 
outputTensor = torch.FloatTensor(outputStorage, 1, torch.LongStorage{#varnames, dataSize})
...
for row=1,dataSize do
  ...
  for col = 1,#outputElementIndices do
    -- swapping of col and row as used in the index of outputTensor
    -- is on purpose (see above)
    outputTensor[{col, row}] = outputElementModules[col].output[outputElementIndices[col]]
  end -- loop over output elements

end -- loop over samples

-- DEBUG
if dataSize < 5000 then
  torch.save("test.t7", outputTensor)
end
